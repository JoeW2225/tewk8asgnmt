...
export default async function Bar( { params }) {
    'use server'
    const id = params.id
    
    const db = connect()
    const pubVenue = (await db.query(`
        SELECT pubsandbars.name, pubsandbars.img_url, pubsandbars.ownership, pubsandbars.established, locations.location
        FROM pubsandbars
        INNER JOIN locations
        ON pubsandbars.location_id = locations.id
        WHERE pubsandbars.id = $1 AND pubsandbars.ispub = false`,[id])).rows[0]
